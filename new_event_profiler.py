# -*- coding: utf-8 -*-
"""
Created on Fri Jan 29 04:07:11 2016
...
def eventprofiler(df_events_arg, d_data, i_lookback=20, i_lookforward=20,
                s_filename='study', b_market_neutral=True, b_errorbars=True, use_actual_close=False,
                s_market_sym='SPY'):
    ''' Event Profiler for an event matix'''
    
    if use_actual_close == False:
        df_close = d_data['close'].copy()
        df_rets = df_close.copy()
    else:
        df_close = d_data['actual_close'].copy()
        df_rets = df_close.copy()
